...
library(dplyr)
library(ggplot2)
library(MASS) # only for modelling later
...
firstComplete=function(fname) {
  summarizeMatch(fname) %>% 
    mutate(complete=ifelse(wktTotal[1]==10,T,ifelse(ballTotal[1]>=120,T,F))) %>% 
    mutate(fname=fname) %>% 
...
## ----twoatop-------------------------------------------------------------
atop(files,matchInfo) %>% filter(type=="T20") -> t20s
atop(t20s$fname,firstComplete) %>% filter(complete) -> complete.t20s
complete.t20s

## ----slow----------------------------------------------------------------
atop(complete.t20s$fname,processMatch) %>% 
  filter(inns=="1st") -> d
d
...
## ------------------------------------------------------------------------
d %>% mutate(ballsLeft=120-totalBalls,
            wktsLeft=10-wktDown) %>%
  filter(!isExtra) -> d1
...
## ------------------------------------------------------------------------
new=expand.grid(wktsLeft=c(1,5,10),ballsLeft=c(1,50,100))
...
env20$lookupTable=aa
str(env20$lookupTable)
...
bigF=function(r,b,w) {
  ...
  tab=get("lookupTable",envir=env20)
  ...
  assign("lookupTable",value=tab,envir=env20)
  return(ans)
}
...
## ----showlookuptable-----------------------------------------------------
env20$lookupTable %>% arrange(bb,ww,rr)
...
for (w in 7:max.wickets) {
  for (r in 0:max.runs) {
    for (b in 1:max.balls) {
      bigF(r,b,w)
    }
